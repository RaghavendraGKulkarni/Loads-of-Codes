...
Problem Statement :- Given a graph with N vertices and M edges, perform Depth First Search Traversal
...
Program Input Format :- N, denoting the number of vertices, M denoting the number of edges and type
                        ...
                        S, the source to start the DFS traversal with.
Program Output Format :- The sequence of vertices representing the DFS Traversal.
**/
...
vector<ll> solve(ll n,vector<ll> adj[],ll s)
{
    ...
    while(next.empty()==false)
    {
        ...
        result.push_back(k);
        next.pop();
        for(auto it=adj[k].rbegin();it!=adj[k].rend();it++)
            if(visited[*it]==false)
            {
                visited[*it]=true;
                next.push(*it);
