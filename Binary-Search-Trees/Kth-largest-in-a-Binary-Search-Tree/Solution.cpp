...
/*
Name        : find()
Description : Recursively finds the kth largest element in a binary search tree
...
void find(TreeNode *root, int &place, TreeNode **result, int k) {
    ...
    // Check if the kth largest element is already found
    if(place > k)
        return;
    ...
    // Search for larger elements in right subtree
    ...
    // Search for larger elements in left subtree
...
/*
Name        : kthLargest()
Description : Initiates the recursion to find the kth largest element in a binary search tree
Arguments   : The pointer to the root node of the tree and the value of k, in that order
Return      : A pointer to the kth largest element node
*/
TreeNode* kthLargest(TreeNode* root, int k) {
    ...
    // Initiate the recursion to find the kth largest element
...
int main() {
    ...
    // Call the subroutine to find the kth largest
    TreeNode* result = kthLargest(root, k);
