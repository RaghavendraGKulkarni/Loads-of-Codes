# Count nodes in a Complete Tree
## Description:
### Given a complete binary tree, the program counts the number of nodes in the tree
## Input Format:
...
## Output Format:
* The first and the only line will contain an integer denoting the number of nodes
## Time Complexity: 
* O(log n)
## Space Complexity: 
* O(log n)