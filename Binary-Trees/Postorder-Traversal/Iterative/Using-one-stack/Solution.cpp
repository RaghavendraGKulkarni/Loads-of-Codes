...
/*
Name        : iterativePostOrder()
Description : Performs Postorder Traversal on a tree iteratively
...
vector<int> iterativePostOrder(TreeNode *root) {
    ...
    // Peform the Postorder traversal iteratively
    TreeNode *cur = root;
    do {
        while(cur) {
            if(cur->right)
        ...
        cur = st.top();
        st.pop();
        if(cur->right && !st.empty() && cur->right == st.top()) {
            st.pop();
            st.push(cur);
            cur = cur->right;
        }
        else {
            traversal.push_back(cur->data);
            cur = NULL;
        }
    }while(!st.empty());
